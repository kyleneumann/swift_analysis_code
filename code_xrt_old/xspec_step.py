...
for dirpath, dirnames, filenames in os.walk(totpath):
    ...
    for i in range(80):
        ...
        if skip_coef > 3:
            ...
            skip_coef += 1
            continue
        elif choice1 == "n" and os.path.isfile("xspec"+str(i)+".log"):
            # print("Not overwriting, skipping 4FGL "+fgl+"/"+str(i))
            continue

        if fgl_bool:
            df = df_sosta.loc[(df_sosta["4FGL"] == "4FGL "+fgl) & (df_sosta["index"] == i)]
        else:
            df = df_sosta.loc[(df_sosta["4FGL"] == fgl) & (df_sosta["index"] == i)]

        if len(df) == 0:
            continue
        elif len(df) != 1:
            df
            raise ValueError("Something broke for 4FGL "+fgl+"/"+str(i))
