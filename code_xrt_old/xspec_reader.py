...
src_arr = df_sosta["4FGL"].to_numpy()

if df_sosta["4FGL"].to_numpy()[0][1:4] == "FGL":
    fgl_bool = True
else:
    fgl_bool = False
...
for dirpath, dirnames, filenames in os.walk(totpath):
    ...
    for i in range(80):
        ...
        if fgl_bool:
            df = df_sosta.loc[df_sosta["4FGL"]=="4FGL "+FGLn].loc[df_sosta["index"] == i]
        else:
        ...
        if len(df) == 0:
            ...
            print(df)
            raise ValueError("Something broke for 4FGL "+FGLn+"/"+str(i))
...
df_total.rename(columns={"4FGL":"SRC"},inplace=True)
